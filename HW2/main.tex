...
\tikzstyle{level 3}=[level distance=15mm, sibling distance=10mm]
\node(0)[s, label=above:Nature]{}
    child{node(1a)[s, label=above:1,label=left:{$S^U_A$},label=right:{$1 - S^U_A$}]{}
        child{node(2aa)[s, label=above:2,label=left:{$\mu_U$}]{}
            child{node(3aaa)[h, label=below:{$5$}]{} edge from parent node[left]{$L$}}
            child{node(3aab)[h, label=below:{$1$}]{} edge from parent node[left]{$R$}}
            edge from parent node[left]{$U$}
        }
        child{node(2ab)[s, label=above:2,label=left:{$\mu_D$}]{}
            child{node(3aba)[h, label=below:{$4$}]{} edge from parent node[left]{$L$}}
            child{node(3abb)[h, label=below:{$0$}]{} edge from parent node[left]{$R$}}
            edge from parent node[left]{$D$}
        }
        edge from parent node[left]{$P$}
        edge from parent node[below]{$A$}
    }
    child{node(1b)[s, label=above:1,label=left:{$S^U_B$},label=right:{$1 - S^U_B$}]{}
        child{node(2ba)[s, label=above:2,label=left:{$1 - \mu_U$}]{}
            child{node(3baa)[h, label=below:{$0$}]{} edge from parent node[left]{$L$}}
            child{node(3baa)[h, label=below:{$2$}]{} edge from parent node[left]{$R$}}
            edge from parent node[left]{$U$}
        }
        child{node(2bb)[s, label=above:2,label=left:{$1 - \mu_D$}]{}
            child{node(3bba)[h, label=below:{$2$}]{} edge from parent node[left]{$L$}}
            child{node(3bba)[h, label=below:{$6$}]{} edge from parent node[left]{$R$}}
            edge from parent node[left]{$D$}
        }
        edge from parent node[right]{$1 - P$}
        edge from parent node[below]{$B$}
    };
\draw[loosely dotted,very thick](2aa)to[out=30,in=-210](2ba);
\draw[loosely dotted,very thick](2ab)to[out=-30,in=210](2bb);
\end{tikzpicture}
\end{center}
...
UU & $5P$ & $2 + 3P$ & $P$ & $2 - P$ \\ 
\hline
UD & $2 + 3P$ & $6 - P$ & $2 - P$ & $6 - 5P$ \\ 
\hline
DU & $4P$ & $2 + 2P$ & $0$ & $2 - 2P$ \\ 
\hline
DD & $2 + 2P$ & $6 - 2P$ & $2 - 2P$ & $ 6 - 6P$ \\ 
\hline
\end{tabular}    
\end{center}\\
The Nash is same as part (a), which is $(UD, RL)$ with payoff of $(2 - P, P - 2)$.
The reason is player-2 has a dominant action regardless of nature choice, so by knowing the player-1 action she can choose the best-response.  
